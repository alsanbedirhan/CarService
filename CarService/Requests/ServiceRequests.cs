...
namespace CarService.Requests
{
    public class ServiceRequests
    {
        ...
        public static async Task<RequestModel<List<SearchCompanyWorkList>>> SearchCompanyWork(decimal MakeId, decimal MakeModelId, string Plaka, string Ad,
            string Soyad, string Isdone, string Isout, DateTime? StartDate = null, DateTime? EndDate = null)
        {
            return await new Request().Post<List<SearchCompanyWorkList>>("companies/search", JsonConvert.SerializeObject(new { MakeId, MakeModelId, Plaka, Ad, Soyad, 
                Isdone, Isout, StartDate = StartDate ?? DateTime.MinValue, EndDate = EndDate ?? DateTime.MinValue }));
        }
